...
// Body: { code, gclid?, gbraid?, wbraid?, utm_source?, utm_medium?, utm_campaign?, utm_content?, utm_term?, landing_url? }
//
...
// checkout-session.js in krob-tracking-stack-main). Bridges a Google Ads
// click (no native "Click to WhatsApp" format, unlike Meta) to a later
// WhatsApp conversation: the landing page embeds `code` in the WhatsApp
// button's pre-filled message text, and the webhook
...
export async function onRequestPost(context) {
  ...
  const { code, gclid, gbraid, wbraid, utm_source, utm_medium, utm_campaign, utm_content, utm_term, landing_url } = body;
  if (!code) {
  ...
  await env.DB
    .prepare(`
      INSERT OR REPLACE INTO ad_click_codes (
        code, gclid, gbraid, wbraid, utm_source, utm_medium, utm_campaign, utm_content, utm_term,
        landing_url, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
    .bind(
      normalizedCode, gclid || null, gbraid || null, wbraid || null,
