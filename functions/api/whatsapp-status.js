...
// business_messaging event so ad delivery can optimize toward it. Shares
// the actual transition logic (status update, anti-duplicate guard, CAPI
// send) with the keyword-triggered path in
// functions/webhook/_whatsapp-core.js via applyStageTransition().
...
export async function onRequestPost(context) {
  ...
  const result = await applyStageTransition({ env, waId: wa_id, newStatus: new_status, source: 'manual', value, currency });
  if (!result.ok) {
