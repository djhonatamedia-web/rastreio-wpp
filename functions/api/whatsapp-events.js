// GET /api/whatsapp-events?key=...&only_ctwa=1&search=5511&limit=50
//
// Dashboard "Eventos" tab — raw webhook-by-webhook, message-by-message log.
// Mirrors the KROB WhatsApp Tracker reference screenshot (ID, recebido,
// nome, telefone, tipo, conteúdo/anúncio). Source: whatsapp_events, joined
// to whatsapp_contacts for push_name/ctwa flag.
...
export async function onRequestGet(context) {
  ...
  try {
    const rows = await env.DB.prepare(`
      ...
      LEFT JOIN whatsapp_contacts c ON e.wa_id = c.wa_id
      ${where}
      ORDER BY e.id DESC
      LIMIT ?
